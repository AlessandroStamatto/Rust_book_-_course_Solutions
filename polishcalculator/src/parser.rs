...
fn evaluate_line(stack: &mut Stack, buf: &str) -> rpn::Result<()> {
    ...
    for token in tokens {
    	try!(match token {
    		...
    		"top" => stack.eval(Op::Top),
    		"true" => stack.push(Elt::Bool(true)),
    		"false" => stack.push(Elt::Bool(false)),
    		number => number.parse::<i32>()
    		          .map_err(|_| rpn::Error::Syntax)
    		          .map(|integer| stack.push(Elt::Int(integer)).unwrap())
    	});
    }
    Ok(())
}
